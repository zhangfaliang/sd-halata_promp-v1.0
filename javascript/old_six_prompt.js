...
function addPrompt(e) {
  ...
  let elementprompt =
    e.target.dataset.pageindex == 1 ? Elements.imgpromt : Elements.txtpromt;
...
function addNPrompt(e) {
  let elementprompt =
    e.target.dataset.pageindex == 1 ? Elements.imgnpromt : Elements.txtnpromt;
  elementprompt.focus();
...
function delLi(domli, cnkey) {
  ...
  let elementprompt = getCurrentPromptsEle();
  elementprompt.value = elementprompt.value.replace(
    selectPrompts[cnkey].en + ",",
    ""
  );

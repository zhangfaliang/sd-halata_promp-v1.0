...
def get_content(text):
    try:
        localtran = bytes.fromhex(
            "68747470733A2F2F646963742E796F7564616F2E636F6D2F772F"
        )
...
class Script(scripts.Script):
    ...
    # 是否是txt2img_prompt 或者img2img_prompt
    def after_component(self, component, **kwargs):
        if (
            component.elem_id == "txt2img_prompt"
            or component.elem_id == "img2img_prompt"
        ):
    ...
    def ui(self, is_img2img):
        ...
        with gr.Row(elem_id=eid):
            with gr.Accordion(label="halara_提示词 v1.0", open=False):
                gr.HTML(
                    '<a class="oldsix-tips" href="https://github.com/thisjam/sd-webui-oldsix-prompt/">【使用说明书】'
                )

                textarea = gr.TextArea(self.json, elem_id=tid, visible=False)
                traninput = gr.Textbox(
                    elem_classes="old-six-traninput",
                    visible=False,
                    show_label="",
                    placeholder="输入中文后按回车翻译,[ALT+Q]键呼出/隐藏",
                )
                ...
                with gr.Column(scale=4, elem_id="oldsix-optit"):
                    btnreload = gr.Button(
                        "🔄",
                        elem_classes="oldsix-reload sm secondary gradio-button svelte-1ipelgc",
                    )
                    gr.Button(
                        "清空正面提示词", variant="secondary", elem_classes="oldsix-clear"
                    )
                    gr.Button(
                        "清空负面提示词", variant="secondary", elem_classes="oldsix-clear"
                    )
                    chDynamic = gr.Checkbox(
                        label="锁定【动态批次】提示词",
                        elem_classes="oldsix-checklock",
                        container=False,
                        scale=1,
                    )
        ...
        btnRandom.click(
            fn=randomPrompt, inputs=None, outputs=[rdtextareaEn, rdtextareaZh]
        )
        chDynamic.select(
            fn=CheckboxChange, inputs=chDynamic, outputs=chDynamic, show_progress=False
        )
        traninput.submit(
            fn=tanslatePromp, inputs=traninput, outputs=[tcache, traninput]
